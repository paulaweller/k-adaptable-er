...
#close(io)
write_result_to_file("source/results/rio/$(product)/results_$(product)_k$(k).csv", result_data)

if save_x
     output = open("source/results/rio/$(product)/individual/sol_rio_$(product)_k$(k).txt", "w")
