...
function solve_bb(K, loc_I, loc_J, W, D, pc)
    ...
    #for it in 1:25
        ...
        theta, x, y, s = solve_scenario_based(tau, loc_I, loc_J, W, D, pc)
        ...
        if theta < theta_i
            ...
            zeta, xi, z = solve_separation_problem(theta, x, y, s, loc_I, loc_J, W, D, pc)
...
"""
    solve_scenario_based(tau, loc_I, loc_J, W, D, pc)

Solve the scenario-based K_adaptable problem for the uncertainty sets tau.
"""
function solve_scenario_based(tau, loc_I, loc_J, W, D, pc)
    I = size(loc_I, 1)
    J = size(loc_J, 1)
    K = length(tau)
...
function solve_separation_problem(theta, x, y, s, loc_I, loc_J, W, D, pc)
    ...
    @variable(us, z[1:K,0:J], Bin)   # violation indicator
    ...
    M = 10^4
    @constraint(us, [k=1:K], sum(z[k,j] for j in 0:J) == 1)
    @constraint(us, [k=1:K], zeta + M*z[k,0] <= M + slack_coeff*sum(s[j,k] for j in 1:J) + sum(c[i,j]*y[i,j,k] for i in 1:I, j in 1:J) - theta)
    @constraint(us, [k=1:K, j=1:J], zeta + M*z[k,j] <= M -( sum(y[i,j,k] for i in 1:I)+s[j,k] - d[j]))

    @objective(us, Max, zeta)
    optimize!(us)

    return round.(Int,getvalue.(zeta)), round.(Int,getvalue.(d)), getvalue.(z)
end