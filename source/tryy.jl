...
k = 1
product = "Medicine"
path = "source/results/rio/$(product)"

# supplies
W_df = CSV.read("source/data/rio_supplies.csv", DataFrame)
W = W_df[W_df[!,1].=="persons total", product][1]

# demand bound per location
D_df = CSV.read("source/data/rio_demand.csv", DataFrame)
...
dist_df = CSV.read("source/data/rio_dist.csv", DataFrame)
...
save_x = true
results, solutions = run_instance(k, instance, tlim=lim, return_solutions=save_x, bb=false, box=false)
...
#close(io)
write_result_to_file("$(path)/results_$(product)_k$(k).csv", result_data)

if save_x
     output = open("$(path)/individual/sol_rio_$(product)_k$(k).txt", "w")
